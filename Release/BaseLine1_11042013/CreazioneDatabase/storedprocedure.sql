...
/
  ...
  CREATE OR REPLACE PROCEDURE "EDITORIALE"."LISTA_ACCOUNT" (P_CURSOR OUT SYS_REFCURSOR )
AS
BEGIN
    OPEN P_CURSOR FOR 
        SELECT * FROM account;
...
, P_PASSWORD IN VARCHAR2  
, P_USERNAME_OUT OUT VARCHAR2  
, P_NOME OUT VARCHAR2  
, P_COGNOME OUT VARCHAR2  
, P_RUOLO OUT VARCHAR2  
) AS 
BEGIN
  SELECT username, nome, cognome INTO P_USERNAME_OUT, P_NOME, P_COGNOME FROM ACCOUNT WHERE username=P_USERNAME_IN and password=P_PASSWORD;
  SELECT nome_gruppo INTO P_RUOLO FROM gruppo_account WHERE username=P_USERNAME_IN;
END LOGIN;
/
